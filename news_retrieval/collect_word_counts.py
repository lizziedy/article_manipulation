...
def get_word_counts(topics = ['immigration', 'stock', 'education'], year_range=range(2007, 2017), base_article_path = 'articles'):
    
    for topic in topics:
        for year in year_range:
            in_file_path = base_article_path + '/' + topic + '/' + str(year) + '_' + str(year + 1) + '.json'
            out_file_path = base_article_path + '/' + topic + '/word_counts_' + str(year) + '_' + str(year + 1) + '.json'
